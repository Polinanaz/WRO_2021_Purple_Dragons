  ...
  <div class="hospital">
    ...
    <div class="center examplex">
      ...
      <div class="square">
        <vs-row>
          <vs-col w='6'>
            <div class="block-header-hospital center-block">
              <div class="block-header-text__hospital">
                <h1>Состояние Больницы</h1>
              </div>
            </div>
            <div class="block__hospital center-block">
              <div class="block__content__hospital">
                <br />
                <h2>Состояние подключения к сети: Больница {{ hospital.status }}</h2>
                <br />
                <h2>Заряд аккумулятора: {{ hospital.battery }}</h2>
                <br />
                <h2>Количество доступной для дома воды: {{ hospital.water }}</h2>
                <br />
                <h2>Текущее потребление Больницы: {{ hospital.total_consumption }}</h2>
              </div>
            </div>
          </vs-col>
          <vs-col w='6'>
            <div class="block-header2-hospital center-block">
              <div class="block-header2-text__hospital">
                <h1>График</h1>
              </div>
            </div>
            <div class="block2__hospital center-block">
              <div class="block__content_2__hospital">
...
export default {
  ...
  mounted () {
    if (this.changeTheme) {
      ...
      let elem = document.getElementsByClassName('footer__hospital')[0]
      elem.style.backgroundColor = '#1e1e1e'
      elem.style.color = '#ffffff'
      document.getElementsByClassName('block__hospital')[0].style.backgroundColor = '#303030'
      document.getElementsByClassName('block__content__hospital')[0].style.color = '#ffffff'
      document.getElementsByClassName('block2__hospital')[0].style.backgroundColor = '#303030'
      document.getElementsByClassName('block__content_2__hospital')[0].style.color = '#ffffff'
      document.getElementsByClassName('block-header-hospital')[0].style.backgroundColor = '#242424'
      document.getElementsByClassName('block-header-text__hospital')[0].style.color = '#ffffff'
      document.getElementsByClassName('block-header2-hospital')[0].style.backgroundColor = '#242424'
      document.getElementsByClassName('block-header2-text__hospital')[0].style.color = '#ffffff'
  ...
  methods: {
    ...
    startTimer () {
      ...
      this.interval = window.setInterval(() => {
        ...
        axios.get('/get_hospital_information')
          .then(response => {
